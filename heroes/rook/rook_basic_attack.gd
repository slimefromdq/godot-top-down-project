...
# (rook_basic_attack.tres). The script only says what's DIFFERENT about
...
# Rook's one piece of custom behaviour: heal for a share of damage dealt.
# The share is a named value in rook_basic_attack.tres ("lifesteal"), so it's
# tunable in the inspector and the debug panel like any other number.
